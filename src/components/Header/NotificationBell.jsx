        ...
        {count > 0 && (
          <div
            className="flex justify-center items-center w-[15px] h-[15px] 
              bg-[#3d9bc7] absolute right-0 rounded-xl 
      ...
      {isOpen && (
        ...
        >
          ...
          {notifications.length > 0 && (
            <div className="border-b border-gray-200 rounded-t-xl px-4 py-2 flex justify-end">
              <button
                onClick={clearNotifications}
                className="text-xs text-[#3d9bc7] hover:text-[#1c759f] font-medium"
              >
                Clear All
              </button>
            </div>
          ...
          <ul className="max-h-64 overflow-y-auto flex-1">
            {notifications.length > 0 ? (
              notifications.map((note, index) => (
                <li
                  key={index}
                  className="px-4 py-2 text-sm text-gray-700 hover:bg-[#90d3f3] transition-colors"
                >
                  {note}
                </li>
              ))
          ...
          {notifications.length > 0 && (
            <div className="border-t border-gray-200 rounded-b-xl px-4 py-2 flex justify-center">
              <button
