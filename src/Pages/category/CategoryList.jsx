...
import { addcategory, categoryList, categoryStatusUpdate, deletecategory, updatecategory } from "../../apis/SuperAdmin";
...
export const CategoryList = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response?.status == 200) {
        setAdminListData(response?.data?.category_list || []);
        setIsLoading(false);
      }
  ...
  const handleStatusToggle = async (id, newStatus) => {
    try {
      const response = await categoryStatusUpdate({ status: newStatus }, id);
      if (response?.status == 200) {
        toast.success(response?.message);
        await fetchCategories();
      }
  ...
  const handleSubmit = async (mode, formdata) => {
    try {
      ...
      if (mode === "add") {
        ...
        response = await updatecategory(submitData, id);
      }
  ...
  return (
    <>
      <div className="p-6  main main_page min-h-screen duration-800 ease-in-out">
        <BreadcrumbsNav customTrail={[{ label: "Category List", path: "/category_list" }]} />
        <div className="flex justify-between items-center">
          ...
          >
            <span className="font-bold">+ </span>ADD CATEGORY
          </button>
        </div>
        {/* Top KPI Cards */}
        {/* <UserTable /> */}
        <div className="mt-4">
          <CategoryTable columns={columns} adminListData={adminListData} isLoading={isLoading} handleEdit={handleEdit} handleDelete={handleDelete} handleStatusToggle={handleStatusToggle} openEditModal={openEditModal} />
