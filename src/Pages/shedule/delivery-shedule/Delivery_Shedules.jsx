...
import { changeStatus_Delivery_Shedule, changeStatus_Pickup_Shedule, editDelivery_Shedules, editPickup_Shedules, getDelivery_Shedules, getPicup_Shedules, } from "../../../apis/SuperAdmin";
...
export const Delivery_Shedules = () => {
    ...
    const fetchDelivery_Shedule = async () => {
        try {
            setIsLoading(true);
            const response = await getDelivery_Shedules();

            if (response?.status == 200) {
                setScheduleList(response?.data?.delivery_schedule_list || []);
                setIsLoading(false);
            }
            ...
            console.error("Error fetching delivery schedule list:", error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleStatusToggle = async (id, newStatus) => {
        try {
            const response = await changeStatus_Delivery_Shedule({ is_closed: newStatus }, id);
            if (response?.status == 200) {
                toast.success(response?.message);
                await fetchDelivery_Shedule();
            }
            else if (response?.response?.data?.status == 401) {
                toast.error(response?.response?.data?.message);
                localStorage.removeItem("user_role");
    ...
    const handleSubmit = async (data) => {
        ...
        try {
            const response = editDelivery_Shedules(id, data)
            console.log("delivery_shedule", response)
    ...
    return (
        <>
            <div className="p-6  main main_page min-h-screen duration-800 ease-in-out">
                <BreadcrumbsNav customTrail={[{ label: "Delivery Schedule", path: "/delivery_schedule" }]} />
                <div className="flex justify-between items-center">

                    <PageTitle title={"Delivery Schedules"} />
                    {/* <button
